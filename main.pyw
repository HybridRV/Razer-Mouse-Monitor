...
ICON_BASE_SIZE = 32
DEVICE_NAME = "Mouse"
battery_status, ICONS = {"percent":"?","state":"Unknown"}, {}
...
def parse_line(line):
    ...
    if m: return {"percent":int(m.group(1)),"state":"Charging" if int(m.group(2)) else "Not Charging"}
    return None
...
def restart_app(icon,item): icon.stop(); os.execl(sys.executable,sys.executable,*sys.argv)

def main():
    load_icons()
    initial_icon=get_icon(0,"Not Charging")
    menu=pystray.Menu(pystray.MenuItem("Restart",restart_app),pystray.MenuItem("Quit",quit_app))
    icon=pystray.Icon("BatteryMonitor",initial_icon,"Battery Monitor",menu)
    status=get_latest()
    if status:
